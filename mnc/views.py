...
from .dbconnect import api_gets,api_post,api_put
...
def ph_prod_cat(request):
    data=api_gets.ph_prod_cata("Select * from ph_product_catagory")
...
@authentication_classes([apikeycheck])
def ph_buss(request):
    data=api_gets.ph_bussiness("Select * from ph_business")
...
def post_ph_business(request):
    try:
        ...
        insert_data=api_post.post_ph_bussiness(query,params)
...
def put_ph_business(request,code):
    try:
        ...
        update_data=api_put.put_ph_business(query,params)
